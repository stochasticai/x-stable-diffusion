...
_login(HfApi(), token="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")
...
async def predict(input_api: Item):
    ...
    with torch.inference_mode(), autocast("cuda"):
        for i, t in tqdm(enumerate(scheduler.timesteps)):
            ...
            inputs = [latent_model_input, torch.tensor([t]).to(torch_device),text_embeddings]
            ...
            latents = scheduler.step(noise_pred.to(torch_device), i, latents)["prev_sample"]
