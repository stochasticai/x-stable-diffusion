...
def get_args():
    ...
    parser.add_argument(
        "--save_path", default="./unet_jit.pt", type=str, help="Nvfuser saved path"
    )
...
def convert(args):
    ...
    latents = torch.randn(
        (args.batch_size, 4, args.img_size[0] // 8, args.img_size[1] // 8)
    )
    latent_model_input = torch.cat([latents] * 2).to(device)
    text_embeddings = (
        torch.randn((args.batch_size, args.max_seq_length, 768)).float().to(device)
    )
    text_embeddings = torch.cat([text_embeddings, text_embeddings])
    timestep_ = torch.tensor([10]).to(device)
    with torch.no_grad():
        with torch.autocast("cuda"):
            traced_applymodel_half = torch.jit.trace(
                unet,
                (latent_model_input, timestep_, text_embeddings),
                check_trace=False,
            )
