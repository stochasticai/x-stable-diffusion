...
class EmbeddingsTests(unittest.TestCase):
    ...
    def test_timestep_flip_sin_cos(self):
        ...
        t1 = torch.cat(
            [t1[:, embedding_dim // 2 :], t1[:, : embedding_dim // 2]], dim=-1
        )
    ...
    def test_sinoid_embeddings_hardcoded(self):
        ...
        t1 = get_timestep_embedding(
            timesteps, embedding_dim, downscale_freq_shift=1, flip_sin_to_cos=False
        )
        # glide, ldm
        t2 = get_timestep_embedding(
            timesteps, embedding_dim, downscale_freq_shift=0, flip_sin_to_cos=True
        )
        # grad-tts
        ...
        assert torch.allclose(
            ...
            torch.tensor(
                [0.9646, 0.9804, 0.9892, 0.9615, 0.9787, 0.9882, 0.9582, 0.9769, 0.9872]
            ),
            1e-3,
        )
        assert torch.allclose(
            ...
            torch.tensor(
                [0.3019, 0.2280, 0.1716, 0.3146, 0.2377, 0.1790, 0.3272, 0.2474, 0.1864]
            ),
            1e-3,
        )
        assert torch.allclose(
...
class Upsample2DBlockTests(unittest.TestCase):
    ...
    def test_upsample_with_conv(self):
        ...
        output_slice = upsampled[0, -1, -3:, -3:]
        expected_slice = torch.tensor(
            [0.7145, 1.3773, 0.3492, 0.8448, 1.0839, -0.3341, 0.5956, 0.1250, -0.4841]
        )
        assert torch.allclose(output_slice.flatten(), expected_slice, atol=1e-3)
    ...
    def test_upsample_with_conv_out_dim(self):
        ...
        output_slice = upsampled[0, -1, -3:, -3:]
        expected_slice = torch.tensor(
            [0.2703, 0.1656, -0.2538, -0.0553, -0.2984, 0.1044, 0.1155, 0.2579, 0.7755]
        )
        assert torch.allclose(output_slice.flatten(), expected_slice, atol=1e-3)
